...
public final class AEstrella {
    ...
    private final int COSTO_DIAGONAL = (int) Math.rint(Math.sqrt((COSTO_VERTICAL * COSTO_VERTICAL) + (COSTO_HORIZONTAL * COSTO_HORIZONTAL)));
    ...
    private static class Comparador implements Comparator<Node>{
        @Override
        // Retorna el valor del costo 'g' (Greedy BFS) y el costo 'h' (funcion heuristica) entre 2 nodos A y B
        public int compare(Node nodoA, Node nodoB){
            return (nodoA.gcost + nodoA.hcost) - (nodoB.gcost + nodoB.hcost);
        }
    }
    ...
    class Node{
     ...
     public void funcionHeuristica(){
         // Costo promedio de moverse en diagonal
